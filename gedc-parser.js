 ...
 * 
 * Uses as liberal a GEDC dialect as I know how to code.
 * 
 ...
 */
//export { GEDCStruct, parseGEDC, g5ConfGEDC, g7ConfGEDC }
...
class GEDCStruct {
   ...
   * @patam {string} newline - how lines should end; defaults to '\n'
   */
  toString(maxlen, newline) {
    ...
    return ans + this.sub.map(s => s.toString(maxlen, newline)).join('')
  }
}
 ...
 */
function parseGEDC(input, config, logger) {
  // level: [0-9]+
  // delim: [ \t\p{Zs}]+
  // xref: (?:[^@#\p{Cc}]|\t)(?:[^@\p{Cc}]|\t)*
  ...
  new GEDCStruct('ADDED', records[1], records[0])
...
console.log(parseGEDC(`0 HEAD
1 GEDC
2 VERS 7.0
1 NOTE @#test@asdfghjklkjhg
2 CONT @@fdsdfhjklkj@hgfdsfghjklkjhfdsdfghjkjhfds
0 TRLR`, {}, console.log).map(e=>e.toString(20)).join(''))
